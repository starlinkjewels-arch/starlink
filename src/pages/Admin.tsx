        ...
        <Card className="w-full max-w-md shadow-2xl border-0">
          ...
          <CardContent className="space-y-5 pt-4">
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              <Button type="submit" size="lg" className="w-full h-11 bg-black hover:bg-gray-800 text-white mt-2" disabled={isSigningIn}>
  ...
  const Sidebar = ({ mobile = false }: { mobile?: boolean }) => (
    <aside className={`${mobile ? 'flex' : 'hidden lg:flex'} flex-col w-64 bg-gray-900 text-white h-screen overflow-y-auto flex-shrink-0 scrollbar-thin-dark`}>
      ...
      <div className="flex items-center gap-3 px-5 py-5 border-b border-gray-700/60">
        ...
        {mobile && (
          <button onClick={() => setSidebarOpen(false)} className="ml-auto text-gray-400 hover:text-white" aria-label="Close menu">
            <X className="h-5 w-5" />
          </button>
        )}
      </div>
      ...
      <div className="p-3 border-t border-gray-700/60">
        <p className="px-3 pb-2 text-xs text-gray-500 truncate" title={user.email ?? undefined}>{user.email}</p>
        <button
